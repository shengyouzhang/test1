package com.zsy.frame.lib.utils;
...
import com.zsy.frame.lib.ui.exception.SYException;
...
public final class SystemTool {
	...
	public static int gc(Context cxt) {
		...
		if (processList != null) for (RunningAppProcessInfo process : processList) {
			...
			if (process.importance > RunningAppProcessInfo.IMPORTANCE_VISIBLE) {
				...
				for (String pkgName : pkgList) {
					SYLoger.debug(TAG, "======正在杀死包名：" + pkgName);
					try {
		...
		SYLoger.debug(TAG, "清理了" + (getDeviceUsableMemory(cxt) - i) + "M内存");
		return count;
	}
	 ...
	 * @description：当前应用的名称
	 * @author samy
	 * @date 2015-1-29 下午12:28:19
	 */
	public static String getCurrProcName(Context context) {
	...
	public static String getAppVersion(Context context) {
		...
		try {
			...
			throw new SYException(SystemTool.class.getName() + "the application not found");
		}
		return version;
	}
	...
	public static String getSign(Context context, String pkgName) {
		try {
			...
			throw new SYException(SystemTool.class.getName() + "the " + pkgName + "'s application not found");
		}
	}
